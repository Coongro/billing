...
- **Trigger name**: `trg_billing_{descriptive_name}`
- **Function name**: `trg_billing_{descriptive_name}_fn`

Where `billing` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_billing_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_billing_audit_log"
AFTER INSERT OR UPDATE ON "module_billing_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_billing_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_billing_*` triggers when the plugin
